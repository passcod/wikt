use std::{collections::HashMap, fs::{create_dir_all, remove_dir_all, File}, path::PathBuf, sync::atomic::{AtomicUsize, Ordering}};
...
fn main() -> Result<()> {
    ...
    match args.action {
            ...
            entries.for_each(|(title, text, store_ref)| {
                ...
                for (name, text) in split_by_section(&LANG_RX, &text).into_iter() {
                    ...
                    for (name, text) in split_by_section(&GRAM_RX, &text).into_iter() {
                        debug!("[{}] lang={:?} gram={:?} section: {:?}", &store_ref, &lang, &name, &text);
                        docs.push(doc!(
                            s_title => title.as_str(),
                            s_text => text.as_str(),
                            s_ref => store_ref.as_u64(),
        ...
        Action::Index(IndexAction::Query { search, limit, titles, full }) => {
            let mut store = blockstore::Store::new(args.store_dir);
            store.open()?;
            ...
            for (score, doc_address) in top_docs {
                ...
                let rid = Ref::from_u64(
                    nameddoc.get("ref").unwrap()[0]
                        .u64_value()
                        .unwrap(),
                );
                ...
                if titles {
                    ...
                    println!(
                        ...
                        score, rid, lang.unwrap_or("?"), gram.unwrap_or("?"), title,
                    );
                } else {
                    ...
                    println!(
                        ...
                        score, rid, lang.unwrap_or("?"), gram.unwrap_or("?"), title, text,
                    );
                }
            }
        }
...
static GRAM_RX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?m)^\s*===([\w\s]+)===\s*$").unwrap()
});
...
fn split_by_section(rx: &Regex, text: &str) -> HashMap<String, String> {
    ...
    positions.iter().enumerate().map(|(i, (name, _, start))| {
        let end = positions.get(i + 1).map(|(_, start, _)| *start).unwrap_or_else(|| text.len() - 1);

        trace!("section part name={:?} start={} end={}", name, start, end);
        (name.to_owned(), text.chars().skip(start - 1).take(end - start).collect::<String>().trim().to_owned())
    }).collect()
}
